...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!data.session) {
      setError('Login failed — check your email for a confirmation link, or disable email confirmation in Supabase.')
      setLoading(false)
      return
    }
    ...
    <div className="min-h-screen flex items-center justify-center px-5" style={{ background: 'var(--bg)' }}>
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xs uppercase tracking-[0.2em] mt-2" style={{ color: 'var(--text-2)' }}>Forge your best self</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <p className="text-center text-sm pt-1" style={{ color: 'var(--text-2)' }}>
            No account?{' '}
            <Link href="/signup" className="font-semibold" style={{ color: 'var(--green)' }}>
              Create one
            </Link>
          </p>
        </form>
